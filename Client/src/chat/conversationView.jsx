...
export function ConversationView({ conversation, currentUser, onBack }) {
  ...
  // Get the other person in the conversation (could be farmer or supplier)
  const otherPerson =
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div
        className={`p-4 border-b border-gray-200 flex items-center ${styles.headerBg}`}
      >
        {onBack && (
          <button
            onClick={onBack}
        ...
        {otherPerson?.profileImage ? (
          ...
          <div
            className={`w-10 h-10 rounded-full ${styles.accentBg} flex items-center justify-center`}
          >
            <User className={`h-5 w-5 ${styles.accentColor}`} />
          </div>
        )}
        <div className="ml-3">
          ...
          <p className="text-xs text-gray-500">
            {otherPerson?.location ? `📍 ${otherPerson.location}` : ""}
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 p-4 overflow-y-auto bg-gray-50">
        {loading ? (
          <div className="flex justify-center items-center h-full">
            <div
              className={`animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 ${styles.loadingColor}`}
            ></div>
          </div>
        ) : messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <div
              className={`w-16 h-16 rounded-full ${styles.accentBg} flex items-center justify-center mb-4`}
            >
              <Send className={`h-6 w-6 ${styles.accentColor}`} />
            </div>
      ...
      >
        <div className="flex items-center">
          ...
          <button
            ...
            className={`${styles.buttonColor} text-white rounded-r-lg px-4 py-2 disabled:opacity-50 disabled:cursor-not-allowed`}
          >
            {isSending ? (
