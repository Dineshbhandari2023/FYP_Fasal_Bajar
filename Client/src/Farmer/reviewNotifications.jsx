...
const ReviewNotifications = () => {
  ...
  useEffect(() => {
    ...
    // main socket reference
    const mainSocket = socketService.socket;
    const reviewSock = socketService.reviewSocket;
    if (!mainSocket || !reviewSock) return;

    // If you want the farmer to see a "new_review" notification,
    // listen for it on the reviewSocket (because that's where "new_review" is emitted)
    ...
    reviewSock.on("new_review", handleNewReview);

    // Existing listeners for main socket:
    const handleReviewSubmitted = (data) => {
      /* ... */
    };
    const handleReviewUpdated = (data) => {
    ...
    // Cleanup
    return () => {
      reviewSock.off("new_review", handleNewReview);
      mainSocket.off("review_submitted", handleReviewSubmitted);
      mainSocket.off("review_updated", handleReviewUpdated);
      mainSocket.off("review_deleted", handleReviewDeleted);
