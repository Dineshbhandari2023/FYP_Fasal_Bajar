...
import { useDispatch, useSelector } from "react-redux";
import { fetchMyProducts } from "../Redux/slice/productSlice";
import {
  fetchPendingOrderItems,
  updateOrderItemStatus,
} from "../Redux/slice/orderSlice";
...
const Body = () => {
  ...
  // Fetch the logged-in user's products and pending orders on mount
  useEffect(() => {
  ...
  // Get pending orders from the orders slice
  const {
    pendingItems = [],
    loading: ordersLoading,
    error: ordersError,
    ...
    <div className="p-6 mt-16 md:ml-64">
      ...
      <div className="bg-white border-b rounded-lg shadow-sm mb-6 p-6">
        ...
        {!prodLoading && products.length === 0 && (
          <p className="mt-2 text-gray-600">No products available.</p>
        )}
              ...
              <div key={order.id} className="mb-4 p-3 border rounded">
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      {order.productName} – {order.quantity} unit(s) at NPR{" "}
                      {order.price}
                    </p>
                    <p className="text-xs text-gray-500">
                      {new Date(order.createdAt).toLocaleDateString()}
