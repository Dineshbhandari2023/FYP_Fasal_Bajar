...
import { fetchCurrentUser } from "../Redux/slice/userSlice"; // adjust the import path as needed

export function Navigation() {
  const dispatch = useDispatch();
  const { userInfo, accessToken } = useSelector((state) => state.user);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const cartItems = 0; // Adjust as needed for dynamic cart functionality

  // Fetch current user info when accessToken exists
  useEffect(() => {
    if (accessToken) {
