...
//         } else {
//           console.warn(
//             "Cannot register as supplier: missing user ID or username"
//           );
//           setError("User data incomplete. Please refresh or log in again.");
//         }
//       })
//       .onDisconnect((reason) => {
...
//       console.error(
//         "User ID is undefined. Make sure you're logged in and the user data is loaded."
//       );
//       // Continue with other operations but skip the ones that require user ID
//     }
...
const RealTimeLocationTracker = ({
  ...
  const dispatch = useDispatch();
  const { userInfo, isAuthenticated, isAuthChecked } = useSelector(
    (state) => state.user
  );
  ...
  }, [dispatch, isAuthenticated, isAuthChecked]);

  // Initialize WebSocket connection
  useEffect(() => {
    if (!isAuthenticated || !userInfo?.id || !isAuthChecked) {
      setError("Authentication required for location tracking");
      return;
    }
    ...
    locationService
      ...
      .onConnect(() => {
        ...
        if (userInfo?.id && userInfo?.username) {
          ...
          console.warn("Cannot register as supplier: missing user data");
          setError("User data incomplete. Please refresh or log in again.");
        }
      })
      .onDisconnect((reason) => {
    ...
    return () => {
      if (userInfo?.id) {
        locationService.setSupplierOffline(userInfo.id);
      }
      locationService.disconnect();
    };
  }, [
  ...
  }, [socketConnected, isAuthenticated, userInfo?.id]);
  ...
  const startTracking = () => {
    ...
    if (!userInfo?.id) {
      setError("Cannot start tracking: User data not loaded");
      return;
    }
  ...
  const stopTracking = () => {
    ...
    // Notify server that supplier is offline
    if (userInfo?.id && socketConnected) {
      locationService.setSupplierOffline(userInfo.id);
    }
    ...
    <div>
      <div className="fixed bottom-4 right-4 z-50">
        <div className="bg-white p-4 rounded-lg shadow-lg">
          ...
          <button
            disabled={!socketConnected || !isAuthenticated || !userInfo?.id}
            onClick={isTracking ? stopTracking : startTracking}
            className={`w-full py-2 rounded text-white font-medium ${
              isTracking
                ? "bg-red-500 hover:bg-red-600"
