...
const { v4: uuidv4 } = require("uuid"); // used to generate unique transaction IDs
const PaymentTransaction = require("./Models/paymentTransaction");
...
// Helper functions (assumed to be defined here)
...
// Updated route to initiate payment
app.post("/initiate-payment", async (req, res) => {
  const { amount, orderId } = req.body;
  let paymentData = {
    amount,
  ...
  try {
    ...
    const reqPayment = JSON.parse(safeStringify(payment));
    if (reqPayment.status === 200) {
      // Create a new PaymentTransaction using orderId, amount, and a unique transactionId
      const transaction = new PaymentTransaction({
        orderId: orderId,
        amount: amount,
      ...
      return res.status(reqPayment.status).send(reqPayment);
    }
  } catch (error) {
...
// Updated route to handle payment status update
app.post("/payment-status", async (req, res) => {
  const { orderId } = req.body; // Extract data from request body
  try {
    // Find the transaction using the correct Sequelize query syntax
    ...
    if (paymentStatusCheck.status === 200) {
      ...
      res.status(paymentStatusCheck.status).json(paymentStatusCheck.data);
    }
  } catch (error) {
    console.error("Error updating transaction status:", error);
    res.status(500).json({ message: "Server error", error: error.message });
