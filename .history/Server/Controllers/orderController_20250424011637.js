...
const processPayment = async (req, res) => {
  ...
  try {
    ...
    const { paymentTransaction } = await initiatePayment(
      totalAmount,
      order.orderNumber,
      buyerId,
      order.paymentMethod,
    ...
    // Since EsewaPaymentGateway handles the redirect, we don't need to send a response here
    // The transaction will be committed after the redirect (handled in success/failure routes)

    // Note: If EsewaPaymentGateway redirects, the following code won't execute until the user returns
    const buyer = await User.findByPk(buyerId, { transaction });
    await sendEmail(
      buyer.email,
      "Payment Initiated",
      `Please complete the payment for order ${order.orderNumber}.`,
      `<p>Please complete the payment for order ${order.orderNumber}.</p>`
    );
    const io = req.app.get("io");
    io.to(`user_${buyerId}`).emit("notification", {
      message: `Payment initiated for order ${order.orderNumber}.`,
      type: "Payment",
    });

    await transaction.commit();

    // We don't send a response here because EsewaPaymentGateway already redirected
  } catch (error) {
    if (transaction) await transaction.rollback();
    console.error("Payment processing error:", error);
    return res.status(500).json({
