      ...
      for (const item of orderItems) {
        ...
        if (!orderMap.has(orderId)) {
          orderMap.set(orderId, {
            ...item.Order.toJSON(),
            OrderItems: [], // Renamed items to OrderItems to match the association
          });
        }
        orderMap.get(orderId).OrderItems.push(item);
      }
...
const processPayment = async (req, res) => {
  ...
  try {
    ...
    // Try parsing as an integer first (for backward compatibility)
    let order;
