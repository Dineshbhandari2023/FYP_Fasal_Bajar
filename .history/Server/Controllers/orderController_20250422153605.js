  ...
  try {
    ...
    for (const item of items) {
      ...
      orderItemsData.push({
        ...
        userId: product.userId, // Use userId instead of farmerId
      });
    }
      ...
      await OrderItem.create(
        {
          ...
          userId: itemData.userId, // Use userId instead of farmerId
          status: "Pending",
        },
        { transaction }
      );
    ...
    const userIds = [...new Set(orderItemsData.map((item) => item.userId))]; // Use userId instead of farmerId
