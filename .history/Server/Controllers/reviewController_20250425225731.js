...
// Create a new review for either a farmer or supplier
const createReview = async (req, res, next) => {
  try {
    ...
    // Use userId from auth middleware (not from body for security)
    const reviewUserId = req.user?.id;
    if (!reviewUserId) {
    ...
    const reviewData = {
      ...
      userId: reviewUserId,
      farmerId: entityType === "Farmer" ? entityIdNum : null,
      supplierId: entityType === "Supplier" ? entityIdNum : null,
    };
    ...
    if (error.name === "SequelizeForeignKeyConstraintError") {
      return next(createError(400, "Invalid user or entity ID"));
    }
