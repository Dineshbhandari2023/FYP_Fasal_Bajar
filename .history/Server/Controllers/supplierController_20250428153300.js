...
const acceptDelivery = async (req, res, next) => {
  ...
  try {
    ...
    const delivery = await Delivery.create({
      ...
      assignedAt: new Date(),
      estimatedPickupTime: new Date(Date.now() + 120 * 60000), // 30 minutes from now
      estimatedDeliveryTime: new Date(Date.now() + 144 * 60000), // 90 minutes from now
    });
      ...
      include: [
        {
          ...
          where: {
            currentLocation: { [Op.ne]: null }, // Only include suppliers with currentLocation
          },
          required: true, // Exclude suppliers without SupplierDetails
        },
      ],
      offset,
      limit,
