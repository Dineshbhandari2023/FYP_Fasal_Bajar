...
const createReview = async (req, res, next) => {
  try {
    ...
    // Create the review (productId and orderId are optional; omit if not used)
...
const getFarmerReviews = async (req, res, next) => {
  try {
    ...
    // Get all reviews for this farmer and explicitly select only existing fields
...
const updateReview = async (req, res, next) => {
  try {
    ...
    // Get the updated review with user info, selecting only existing fields
...
const getFarmerRating = async (req, res, next) => {
  try {
    ...
    // Get all ratings for this farmer using only the "rating" field
    const reviews = await Review.findAll({
      attributes: ["rating"],
      where: { farmerId },
    });
