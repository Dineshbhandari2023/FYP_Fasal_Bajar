...
const createOrder = async (req, res, next) => {
  const transaction = await sequelize.transaction();
  let newOrder; // to hold the created order
  let payMethod; // declare variable outside so it's accessible later
  ...
  try {
    ...
    payMethod = paymentMethod; // assign paymentMethod to a variable available outside
    ...
    // Send notifications (Socket.IO & emails) here...
    const io = req.app.get("io");
    io.to(`user:${buyerId}`).emit("order:created", {
      orderId: newOrder.id,
      orderNumber: newOrder.orderNumber,
  ...
  // Payment integration (outside the try block)
