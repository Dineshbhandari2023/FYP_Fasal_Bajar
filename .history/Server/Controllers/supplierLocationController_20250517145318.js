...
exports.updateSupplierLocation = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // If we have an active socket connection, update the real-time location too
    const locationSocket = req.app.get("locationSocket");
    if (locationSocket && locationSocket.namespace) {
      locationSocket.namespace.emit("supplier:location", {
        supplierId,
        latitude,
        longitude,
