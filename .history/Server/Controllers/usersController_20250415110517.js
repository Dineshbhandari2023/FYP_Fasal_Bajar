...
transporter.verify((error, success) => {
  if (error) {
    ...
    console.log("Fasal Bajar server running...");
  }
});

// Helper: construct relative URL for uploaded image
const getProfileImageUrl = (file) => {
  // Updated to match the static files path in server.js
...
//     // Instead of reading file as buffer, store the relative URL.
//     let profileImage = null;
//     if (req.file) {
//       profileImage = getProfileImageUrl(req.file);
//     }
...
//       subject: "Welcome to Fasal Bajar!",
//       html: `
//         <html>
...
//       IsSuccess: true,
//       ErrorMessage: [],
//       Result: { message: "User registered successfully", user_data: userObj },
//     });
//   } catch (error) {
//     console.error(error);
...
// Unified Register a new user (Farmer, Buyer, or Supplier)
const registerUser = async (req, res, next) => {
  const { username, email, role, contact_number, location, password } =
    req.body;
  ...
  // Validate role: must be Farmer, Buyer, or Supplier
  if (role !== "Farmer" && role !== "Buyer" && role !== "Supplier") {
    return next(
      createError(400, "Role must be either 'Farmer', 'Buyer', or 'Supplier'.")
    );
  }

  // Validate username, email, and password formats
  if (!usernameRegex.test(username)) {
    return next(
      createError(
        400,
  ...
  try {
    // Check for duplicates: username, email, and contact number
    const existingUser = await User.findOne({ where: { username } });
    if (existingUser)
      return next(createError(400, "Username is already taken."));
    ...
    // Prepare a customized welcome email based on role
    let subject, htmlContent;
    if (role === "Supplier") {
      subject = "Welcome to Fasal Bajar as a Supplier!";
      htmlContent = `<html>
        <body style="font-family: Arial, sans-serif;">
          <h1>Welcome, ${username}!</h1>
          <p>Thank you for joining Fasal Bajar as a Supplier – you play a crucial role in connecting farmers and buyers.</p>
          <p>Please complete your supplier profile by adding your vehicle and license details to start accepting delivery requests.</p>
          <p>Best regards,</p>
          <p>Fasal Bajar Team</p>
        </body>
      </html>`;
...
// const registerSupplierUser = async (req, res, next) => {
//   const { username, email, contact_number, location, password } = req.body;

//   if (!username || !email || !contact_number || !location || !password) {
//     return next(createError(400, "All fields are required."));
//   }
...
//   } catch (error) {
//     console.error(error);
//     next(createError(500, "Server error while creating new supplier."));
//   }
// };
...
const forgotPassword = async (req, res, next) => {
  ...
  try {
    ...
    const mailOptions = {
      to: user.email,
      from: process.env.EMAIL_USER,
      subject: "Password Reset Request - Fasal Bajar",
      html: `
        <html>
          <body style="font-family: Arial, sans-serif;">
...
const resetPassword = async (req, res, next) => {
  ...
  try {
    ...
    const confirmationMailOptions = {
      to: user.email,
      from: process.env.EMAIL_USER,
      subject: "Password Changed Successfully - Fasal Bajar",
      html: `
        <html>
          <body style="font-family: Arial, sans-serif;">
...
// Update user data (username, email and profileImage)
const updateUser = async (req, res, next) => {
  ...
  try {
    ...
    // Update profileImage using the relative URL with the correct prefix
    if (req.file) {
...
// Get all farmers with full details
const getAllFarmers = async (req, res, next) => {
  try {
    ...
    console.error(error);
    next(createError(500, "Server error while fetching farmers."));
  }
};
