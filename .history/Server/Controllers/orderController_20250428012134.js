...
const getFarmerOrders = async (req, res, next) => {
  try {
    const userId = req.user.user_id; // From JWT (set by authMiddleware)
    const orders = await Order.findAll({
      include: [
        {
          model: OrderItem,
          include: [{ model: Product, where: { userId } }], // Products owned by the farmer
        },
        { model: User, as: "buyer" },
      ],
    });
    ...
    next({
      status: 500,
      message: `Failed to fetch farmer orders: ${error.message}`,
    });
  }
};
