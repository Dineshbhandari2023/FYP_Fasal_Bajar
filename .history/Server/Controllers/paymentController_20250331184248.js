...
exports.initiateEsewaPayment = async (req, res, next) => {
  try {
    ...
    if (!amount || !transaction_uuid) {
      return res
        .status(400)
        .json({ message: "Amount and transaction_uuid are required." });
    }
    ...
    const esewaPaymentUrl = `${process.env.ESEWA_GATEWAY_URL}/api/epay/main/v2/form`;
    ...
    res.status(200).json({
      ...
      data: {
        ...
        product_service_charge: 0,
        product_delivery_charge: 0,
        success_url: `${process.env.SERVER_URL}/esewa-payment-success`,
        failure_url: `${process.env.SERVER_URL}/esewa-payment-failure`,
        signed_field_names: "total_amount,transaction_uuid,product_code",
        signature,
      },
    });
  ...
  try {
    ...
    if (response.data.status === "COMPLETE") {
      ...
      await PaymentTransaction.update(
        { status: "Completed", paymentMethod: "Online Payment" },
        { where: { productId: transaction_uuid } }
      );
