...
const createReview = async (req, res, next) => {
  try {
    ...
    req.io.to(`farmer_${farmerId}`).emit("new_review", reviewWithUser);
...
const updateReview = async (req, res, next) => {
  try {
    ...
    // Emit socket event for real-time updates
    req.io.to(`farmer_${review.farmerId}`).emit("update_review", updatedReview);
...
const deleteReview = async (req, res, next) => {
  try {
    ...
    // Emit socket event for real-time updates
    req.io.to(`farmer_${farmerId}`).emit("delete_review", reviewId);
