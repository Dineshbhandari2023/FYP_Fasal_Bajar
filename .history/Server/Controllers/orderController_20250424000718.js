...
const processPayment = async (req, res) => {
  ...
  try {
    ...
    console.log("Parsed Order ID:", parsedOrderId); // Debug: Log the parsed ID
    if (!isNaN(parsedOrderId)) {
      order = await Order.findByPk(parsedOrderId, {
        include: [{ model: OrderItem }],
        transaction,
      });
      console.log("Order found by ID:", order); // Debug: Log the order found by ID
    }

    if (!order) {
      console.log("Attempting to find order by orderNumber:", orderId); // Debug: Log the orderId
      order = await Order.findOne({
        where: { orderNumber: orderId },
        include: [{ model: OrderItem }],
        transaction,
      });
      console.log("Order found by orderNumber:", order); // Debug: Log the order found by orderNumber
    }
