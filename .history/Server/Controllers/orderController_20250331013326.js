...
const createOrder = async (req, res, next) => {
  ...
  try {
    ...
    // Create the order record inside the transaction
    ...
    // Send confirmation emails to buyer and farmers (omitted for brevity; your existing code is fine)
  } catch (error) {
  ...
  if (paymentMethod === "Online Payment") {
    try {
      ...
      const paymentUrl = paymentResponse.data.url;
      // Return order details along with paymentUrl so frontend can redirect
      return res.status(201).json({
        StatusCode: 201,
        IsSuccess: true,
        ErrorMessage: [],
      ...
      // Since order is already created, we cannot roll back.
      // You can mark the order as 'Payment Failed' or simply return an error.
      return res.status(500).json({
        StatusCode: 500,
        IsSuccess: false,
        ErrorMessage: [
          "Payment integration failed: " +
            paymentError.response?.data?.error_message || paymentError.message,
        ],
