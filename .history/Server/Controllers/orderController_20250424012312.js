...
const processPayment = async (req, res) => {
  ...
  try {
    ...
    // Initiate payment with eSewa using esewajs
    ...
    // Since EsewaPaymentGateway handles the redirect, we don't send a response here
    // The transaction will be committed in the success/failure routes
  } catch (error) {
    if (transaction) await transaction.rollback();
    console.error("Payment processing error:", error);
    return res.status(500).json({
