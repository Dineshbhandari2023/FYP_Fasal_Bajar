...
const generateOrderNumber = async () => {
  ...
  const existingOrder = await Order.findOne({ where: { orderNumber } });
  if (existingOrder) {
    // Recursively generate a new one if needed
    return generateOrderNumber();
  }
  return orderNumber;
};

// Create a new order with payment integration and Socket.IO notifications
const createOrder = async (req, res, next) => {
  const transaction = await sequelize.transaction();
  let newOrder; // will hold the created order
  let payMethod; // holds the payment method
  ...
  try {
    ...
    payMethod = paymentMethod; // assign to outer variable
    ...
    newOrder = await Order.create(
      {
        ...
        totalAmount,
        status: "Processing",
        paymentMethod: payMethod || "Cash on Delivery",
        paymentStatus: "Pending",
        shippingAddress,
        city,
        state,
    ...
    // Send notifications (e.g., Socket.IO, emails) if needed
    const io = req.app.get("io");
    io.to(`user:${buyerId}`).emit("order:created", {
      orderId: newOrder.id,
      orderNumber: newOrder.orderNumber,
    ...
    // (Email notifications code omitted for brevity)
  } catch (error) {
  ...
  if (payMethod === "Online Payment") {
    try {
      // Generate a unique transaction UUID (e.g., order number + timestamp)
      const transactionUUID = `${newOrder.orderNumber}-${Date.now()}`;

      // Call initiate-payment endpoint with the new unique transactionUUID
      const paymentResponse = await axios.post(
        `${process.env.SERVER_URL}/initiate-payment`,
        { amount: totalAmount, orderId: newOrder.id, transactionUUID }
      );
...
// Update order item status (farmer accepts/declines)
const updateOrderItemStatus = async (req, res, next) => {
  try {
    const { itemId } = req.params;
    const { status, farmerNotes } = req.body;
...
const getPendingOrderItems = async (req, res, next) => {
  try {
    ...
    const pendingItems = await OrderItem.findAll({
      where: { farmerId: farmerId, status: "Pending" },
      include: [
        {
          model: Order,
          include: [
        ...
        html: `
          <html>
            <body style="font-family: Arial, sans-serif;">
              ...
              ${
                status === "Shipped"
                  ? `<p>Your order is on its way! Expect delivery in 2-3 business days.</p>`
                  : status === "Delivered"
                  ? `<p>Your order has been delivered. Thank you for shopping with FasalBajar!</p>`
                  : status === "Cancelled"
                  ? `<p>Your order has been cancelled. Please contact support if needed.</p>`
                  : ""
              }
            </body>
          </html>
          ...
          include: [
            {
              model: OrderItem,
              include: [
                { model: Product, attributes: ["productName", "image"] },
              ],
            },
          ],
        }),
...
// Get order statistics for the farmer dashboard
const getOrderStats = async (req, res, next) => {
  try {
    const userId = req.user.id;
    const user = await User.findByPk(userId);
    ...
    const declinedCount = await OrderItem.count({
      where: { farmerId: userId, status: "Declined" },
    });
    const recentOrders = await OrderItem.findAll({
      where: { farmerId: userId },
      limit: 5,
      ...
      include: [
        {
          model: Order,
          include: [{ model: User, as: "buyer", attributes: ["username"] }],
        },
        { model: Product, attributes: ["productName", "unit"] },
      ],
    });
    const currentDate = new Date();
    const sixMonthsAgo = new Date();
