...
// const { generateAccessToken } = require("../Util/auth"); // Using only access token
...
exports.registerUser = async (req, res, next) => {
  ...
  try {
    ...
    if (existingUser) {
      return next(createError(400, "Email or name already in use."));
    }
    ...
    await transporter.sendMail({
      ...
      html: `<p>Hello ${name}, welcome to Fasal Bajar!</p>`,
    });

    res.status(201).json({
      message: "User registered successfully.",
      user: { id: user.userId, name, email, role, location },
      token,
    });
  } catch (err) {
