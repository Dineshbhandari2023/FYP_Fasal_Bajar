...
const processPayment = async (req, res, next) => {
  try {
    ...
    if (!user || user.role !== "Buyer") {
      return next(createError(403, "Only buyers can make payments"));
    }
    ...
    if (!order) {
      return next(
        createError(404, "Order not found or not eligible for payment")
      );
    }

    // Check if any items are accepted
    const orderItems = await OrderItem.findAll({
      where: {
        orderId: order.id,
        status: "Accepted",
    ...
    if (orderItems.length === 0) {
      return next(createError(400, "No accepted items to pay for"));
    }
    ...
    next(
      createError(
        500,
        `Server error while processing payment: ${error.message}`
      )
    );
  }
};
