...
exports.initiatePayment = async (req, res) => {
  ...
  const data = `total_amount=${amount},transaction_uuid=${productId},product_code=${process.env.ESEWA_PRODUCT_CODE}`;
  ...
  res.json({
    ...
    params: {
      ...
      product_service_charge: 0,
      product_delivery_charge: 0,
      success_url: `${process.env.SERVER_URL}/esewa-success`,
      failure_url: `${process.env.SERVER_URL}/esewa-failure`,
      signed_field_names: "total_amount,transaction_uuid,product_code",
      signature,
    },
  });
...
exports.paymentSuccess = async (req, res) => {
  ...
  const verifyUrl = `${process.env.ESEWA_GATEWAY_URL}/api/epay/transaction/status`;
  ...
  if (response.data.status === "COMPLETE") {
    await PaymentTransaction.update(
      { status: "Completed" },
      { where: { productId: decoded.transaction_uuid } }
    );

    await Order.update(
      { paymentStatus: "Completed", status: "Confirmed" },
      { where: { orderNumber: decoded.transaction_uuid } }
    );

    res.redirect(`http://localhost:3000/payment-success?data=${encodedData}`);
  } else {
