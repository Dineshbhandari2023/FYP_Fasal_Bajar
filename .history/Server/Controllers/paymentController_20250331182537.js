...
const initiatePayment = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(200)
        .json({ paymentUrl: paymentResponse.data.payment_url });
    } else {
      console.error("Esewa gateway response error:", paymentResponse.data);
      return res
        .status(400)
...
const checkPaymentStatus = async (req, res) => {
  const { productId, shippingAddress, city, state, pinCode, notes } = req.body;
  ...
  try {
    ...
    if (
      ...
      const order = await Order.create({
        ...
        pinCode,
        notes,
        buyerId: req.user.id, // Ensure you get this from auth middleware
        status: "Confirmed",
      ...
      transaction.status = paymentStatusResponse.data.status || "Failed";
      await transaction.save();
      return res.status(400).json({ error: "Payment not completed." });
    }
  } catch (error) {
