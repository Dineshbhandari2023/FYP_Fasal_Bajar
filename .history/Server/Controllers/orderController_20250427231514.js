...
const getFarmerOrderItems = async (req, res, next) => {
  try {
    const farmerId = req.user.id;
    console.log("getFarmerOrderItems: Farmer ID:", farmerId);

    const farmer = await User.findByPk(farmerId);
    console.log("getFarmerOrderItems: Farmer:", farmer);
    if (!farmer || farmer.role !== "Farmer") {
      console.log("getFarmerOrderItems: Access denied, not a farmer");
      return next(createError(403, "Only farmers can access this endpoint"));
    }
    ...
    console.log("getFarmerOrderItems: Order items:", orderItems);
    ...
    console.error("getFarmerOrderItems error:", error);
    next(
      createError(
        500,
