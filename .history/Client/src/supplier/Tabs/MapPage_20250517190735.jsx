    ...
    try {
      ...
      routingControlRef.current.on("routesfound", (e) => {
        try {
          ...
          console.error("Error processing route layers:", error);
        }
      });
...
export default function SupplierMapPage() {
  ...
  const { suppliers, activeDeliveries, supplierDetails, user } = useSelector(
  ...
  // Log OSRM server recommendation
  useEffect(() => {
    console.info(
      "Consider setting up your own OSRM server or using a paid service like Mapbox for production."
    );
  }, []);
  ...
  // Select first delivery by default and center map
  useEffect(() => {
  ...
  useEffect(() => {
    locationService
      .onSupplierLocationUpdate(({ supplierId, latitude, longitude }) => {
      ...
      .onActiveSuppliersList((activeSuppliers) => {
        const locations = {};
                        ...
                        <Marker position={pickupLocation} icon={pickupIcon}>
                          <Popup>
                            <div className="text-center">
                              ...
                              <p className="text-xs text-gray-600">
                                Status:{" "}
                                {statusConfig[selectedDelivery?.status]
                                  ?.label || "N/A"}
                              </p>
                            </div>
                          </Popup>
