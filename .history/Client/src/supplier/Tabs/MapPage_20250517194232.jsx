...
import { MapPin, Navigation, Truck, LocateFixed, Route, Package, Clock, Layers } from "lucide-react";
import { MapContainer, TileLayer, Marker, Popup, useMap, ZoomControl } from "react-leaflet";
import L from "leaflet";
import "leaflet/dist/leaflet.css";
import "leaflet-routing-machine";
import locationService from "../../map/webSocketLocationService";
import { getAllSuppliers, getActiveDeliveries } from "../../Redux/slice/supplierSlice";
...
function RouteLayer({ pickupLocation, deliveryLocation, currentLocation }) {
  ...
  useEffect(() => {
    ...
    // Validate waypoints
    const waypoints = [currentLocation, pickupLocation, deliveryLocation].filter(
      (loc) => loc && Array.isArray(loc) && loc.length === 2 && !isNaN(loc[0]) && !isNaN(loc[1])
    );
...
export default function MapPage() {
  ...
  const { suppliers, activeDeliveries, supplierDetails } = useSelector((state) => state.supplier);
  ...
  const debouncedUpdateLocation = useRef(
    debounce((supplierId, latitude, longitude, heading, speed) => {
      locationService.updateLocation(supplierId, latitude, longitude, heading, speed);
    }, 5000)
  ).current;
    ...
    if (navigator.geolocation) {
      watchIdRef.current = navigator.geolocation.watchPosition(
        (position) => {
          ...
          if (user?.id && locationService.isConnected()) {
            debouncedUpdateLocation(user.id, latitude, longitude, heading || null, speed || null);
          }
          setIsLoaded(true);
        },
        (error) => {
    ...
    locationService
      ...
      .onConnect(() => {
        ...
        if (user?.id && user?.username) {
          locationService.registerAsSupplier(user.id, user.username, supplierDetails?.serviceArea);
        }
      })
      .onError((error) => {
  ...
  const parseLocation = (location) => {
    ...
    try {
      if (typeof location === "string") {
        ...
        return [Number.parseFloat(location.latitude), Number.parseFloat(location.longitude)];
      }
    } catch (error) {
  ...
  const getDeliveryLocations = () => {
    ...
    if (selectedDelivery.pickupLocation?.address) {
      ...
      pickupLocation = parseLocation(selectedDelivery.OrderItem.farmer.location);
    }
    ...
    if (selectedDelivery.deliveryLocation?.address) {
      deliveryLocation = parseLocation(selectedDelivery.deliveryLocation.address);
    } else if (selectedDelivery.OrderItem?.Order?.buyer?.location) {
      deliveryLocation = parseLocation(selectedDelivery.OrderItem.Order.buyer.location);
    }
  ...
  const statusConfig = {
    ...
    In_ Transit: { label: "In Transit" },
    Delivered: { label: "Delivered" },
    Failed: { label: "Failed" },
    Cancelled: { label: "Cancelled" },
  };

  const { pickup: pickupLocation, delivery: deliveryLocation } = getDeliveryLocations();
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="p-4 sm:p-6 border-b">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Active deliveries in your service area</p>
                </div>
              </div>
              <div className="flex mt-4 border-b overflow-x-auto">
                <button
                  className={`px-4 py-2 text-sm font-medium whitespace-nowrap ${
                    activeTab === "current" ? "border-b-2 border-green-600 text-green-600" : "text-gray-500"
                  }`}
              ...
              >
                {!isLoaded ? (
                  ...
                  >
                      ...
                      <Marker position={pickupLocation} icon={pickupIcon}>
                        <Popup>
                          <div className="text-center">
                            ...
                            <p className="text-xs text-gray-600">
                              Status: {statusConfig[selectedDelivery?.status]?.label || "N/A"}
                            </p>
                          </div>
                        </Popup>
                      </Marker>
                    ...
                    {Object.entries(supplierLocations).map(([supplierId, { latitude, longitude }]) => (
                      <Marker key={supplierId} position={[latitude, longitude]} icon={supplierIcon}>
                        <Popup>
                          <div className="text-center">
                ...
                <div className="p-3 border rounded-lg">
                  ...
                  <div className="text-lg font-bold">
                    {currentLocation && deliveryLocation
                      ? `${calculateDistance(currentLocation[0], currentLocation[1], deliveryLocation[0], deliveryLocation[1])} km`
                      : "N/A"}
                  </div>
                </div>
                <div className="p-3 border rounded-lg">
                  <div className="text-sm font-medium text-gray-500">Active Orders</div>
                  <div className="text-lg font-bold">{activeDeliveries.length} orders</div>
                </div>
                <div className="p-3 border rounded-lg">
                  <div className="text-sm font-medium text-gray-500">Location Status</div>
                  <div className="text-lg font-bold flex items-center">
                    <span className={`h-2 w-2 rounded-full ${currentLocation ? "bg-green-500" : "bg-red-500"} mr-2`}></span>
          ...
          <div className="bg-white rounded-lg shadow-sm border">
            <div className="p-4 sm:p-6 border-b">
              ...
              <p className="text-sm text-gray-500">Select a delivery to view on map</p>
            </div>
            <div className="p-4">
              <div className="space-y-3 max-h-[calc(50vh-100px)] overflow-y-auto">
                {activeDeliveries.length > 0 ? (
                  activeDeliveries.map((delivery) => (
                    <div
                      ...
                      className={`border rounded-lg p-3 cursor-pointer transition-colors ${
                        selectedDelivery?.id === delivery.id ? "border-green-500 bg-green-50" : "hover:bg-gray-50"
                      }`}
                      onClick={() => {
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="font-medium text-sm">{delivery.deliveryNumber}</h4>
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            Order: {delivery.OrderItem?.Order?.orderNumber || "N/A"}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="mt-2 grid grid-cols-2 gap-2">
                        <div className="text-xs">
                          ...
                          <p className="truncate">
                            {delivery.pickupLocation?.address || delivery.OrderItem?.farmer?.location || "N/A"}
                          </p>
                        </div>
                        <div className="text-xs">
                          ...
                          <p className="truncate">
                            {delivery.deliveryLocation?.address || delivery.OrderItem?.Order?.buyer?.location || "N/A"}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="mt-2 text-xs flex items-center gap-1">
                        ...
                        <span className="truncate">
                          ...
                          {delivery.OrderItem?.quantity && ` (${delivery.OrderItem.quantity})`}
                        </span>
                      </div>
                    </div>
                  ))
                  ...
                  <div className="text-center py-8">
                    ...
                    <h3 className="mt-2 text-lg font-medium text-gray-500">No active deliveries</h3>
