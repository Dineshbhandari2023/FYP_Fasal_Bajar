...
function RouteLayer({ pickupLocation, deliveryLocation, currentLocation }) {
  ...
  const cleanupRouting = useCallback(() => {
    ...
    // Also clean up any remaining routing layers
    if (routingLayerRef.current) {
      try {
        map.removeLayer(routingLayerRef.current);
      } catch (error) {
        console.error("Error removing routing layer:", error);
      }
  ...
  useEffect(() => {
    // First clean up any existing routing control
    cleanupRouting();

    // Validate all required locations exist and are valid
    ...
    // Validate waypoints are valid arrays with numeric coordinates
    ...
    try {
      // Create waypoints from valid locations
      const waypoints = [
      ...
      // Create a routing layer for the map
      routingLayerRef.current = L.layerGroup().addTo(map);

      // Create a custom plan with the waypoints
  ...
  }, [map, pickupLocation, deliveryLocation, currentLocation, cleanupRouting]);
