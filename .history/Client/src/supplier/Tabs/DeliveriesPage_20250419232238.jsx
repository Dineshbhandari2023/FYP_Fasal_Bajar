...
export default function DeliveriesPage() {
  ...
  const { activeDeliveries, deliveryHistory, loading, error } = useSelector(
  ...
  useEffect(() => {
    ...
    dispatch(getDeliveryHistory({ page: 1, limit: 20 }));
  }, [dispatch]);
  ...
  const handleStatusSubmit = (e) => {
    ...
    formData.append("deliveryId", statusUpdateData.deliveryId);
    formData.append("status", statusUpdateData.status);
    if (statusUpdateData.notes)
      formData.append("notes", statusUpdateData.notes);
    if (statusUpdateData.proofOfDelivery)
      formData.append("proofOfDelivery", statusUpdateData.proofOfDelivery);
                    ...
                    >
                      ...
                      <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="border rounded-md p-3">
                          ...
                          <p className="text-sm font-medium">
                            {delivery.Order?.buyer?.username || "N/A"}
                          </p>
                          <p className="text-sm text-gray-500">
                            {delivery.Order?.buyer?.location || "N/A"}
                          </p>
                          {delivery.Order?.buyer?.contact_number && (
                            <a
                              href={`tel:${delivery.Order.buyer.contact_number}`}
                              className="mt-2 inline-flex items-center text-xs text-blue-600"
                            >
                              <Phone className="h-3 w-3 mr-1" />
                              {delivery.Order.buyer.contact_number}
