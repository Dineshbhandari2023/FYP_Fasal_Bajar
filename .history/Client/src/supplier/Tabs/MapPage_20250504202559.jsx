...
export default function SupplierMapPage() {
  const dispatch = useDispatch();
  const { activeDeliveries } = useSelector((state) => state.supplier);
  const [isLoaded, setIsLoaded] = useState(false);
  const [activeTab, setActiveTab] = useState("current");
  const [selectedDelivery, setSelectedDelivery] = useState(null);
  const [mapCenter, setMapCenter] = useState([26.660944, 87.280769]);
  const [zoom, setZoom] = useState(13);
  const [currentLocation, setCurrentLocation] = useState(null);
  ...
  useEffect(() => {
    if (activeDeliveries.length && !selectedDelivery) {
      setSelectedDelivery(activeDeliveries[0]);
    }
  }, [activeDeliveries, selectedDelivery]);
  ...
  const toggleLayer = () => {
    setMapLayers((prev) => (prev === "streets" ? "satellite" : "streets"));
  };
  const startNavigation = () => {
    if (!selectedDelivery) return;
    const dest = ["Assigned", "Pickup_In_Progress"].includes(
        ...
        <div className="space-y-6">
          <header className="flex justify-between items-center">
            ...
            <div className="flex gap-2">
              ...
              >
                <Layers className="h-4 w-4" />
                {mapLayers === "streets" ? "Satellite" : "Streets"}
              </button>
              <button
                onClick={startNavigation}
                disabled={!selectedDelivery}
          ...
          <div className="lg:flex lg:space-x-6">
            {/* Map Section */}
            <div className="lg:flex-1 bg-white rounded shadow">
              <nav className="flex space-x-2 border-b p-4">
                {["current", "all", "optimize"].map((tab) => (
                  <button
                    key={tab}
              ...
              <div className="p-4">
                <div
                  className="h-[50vh] rounded overflow-hidden"
                  ref={leafletMapRef}
                >
                  {!isLoaded ? (
                ...
                <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="p-3 border rounded">
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`h-2 w-2 rounded-full \${currentLocation ? 'bg-green-500' : 'bg-red-500'}`}
                      />
                      {currentLocation ? "Active" : "Inactive"}
                    </div>
                  </div>
            ...
            {/* List Section */}
            <aside className="lg:w-1/3 bg-white rounded shadow p-4">
              <h2 className="text-lg font-medium mb-2">Active Deliveries</h2>
                ...
                {activeDeliveries.map((del) => (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      <span className="font-medium">{del.deliveryNumber}</span>
