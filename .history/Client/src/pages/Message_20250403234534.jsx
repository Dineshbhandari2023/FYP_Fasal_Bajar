...
const Message = () => {
  ...
  // Sample data for conversations list
  const conversations = [
    {
      id: 1,
      name: "Sarah Wilson",
  ...
  useEffect(() => {
    socket.on("new_message", (data) => {
      // Check if the incoming message belongs to the active conversation
      if (
        data.sender.id === activeConversation.id ||
        data.receiver.id === activeConversation.id
      ) {
        // Append the new message to the activeMessages state
        setActiveMessages((prevMessages) => [
          ...prevMessages,
          {
            ...
            isSentByMe: data.sender.id === socket.auth?.user_id, // Adjust this based on your auth logic
          },
        ]);
      }
    });
  ...
  const handleSendMessage = (e) => {
    ...
    if (message.trim()) {
      ...
      // Optionally, add the sent message immediately to the local state
      setActiveMessages((prevMessages) => [
        ...prevMessages,
        {
