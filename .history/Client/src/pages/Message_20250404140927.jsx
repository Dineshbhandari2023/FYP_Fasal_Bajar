...
const Message = () => {
  ...
  // activeConversation is the full conversation object as returned from the API.
  ...
  useEffect(() => {
    if (activeConversation) {
      // The conversation object includes the other party as "user" or "farmer"
      const otherPerson = activeConversation.user || activeConversation.farmer;
      ...
      if (otherPersonId) {
        dispatch(fetchMessages(otherPersonId))
          ...
          .then((fetchedMessages) => {
            ...
            // If you want to mark messages as read, you can dispatch that here.
            // For example: dispatch(markMessagesAsRead(otherPersonId));
          })
          .catch((error) => {
  ...
  useEffect(() => {
    const handleNewMessage = (data) => {
      if (activeConversation) {
        // Determine the other party from the active conversation
        const otherPerson =
          activeConversation.user || activeConversation.farmer;
        const otherPersonId = Number(otherPerson?.id);
  ...
  const handleSendMessage = (e) => {
    ...
    if (message.trim() && activeConversation) {
      ...
      // Emit the send_message event via socket
  ...
  const filteredConversations = (
    ...
    const otherPerson = conv.user || conv.farmer;
    return (otherPerson.username || "")
      .toLowerCase()
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-3">
                      ...
                      <p className="text-xs text-green-500">
                        {otherPerson.online ? "Online" : "Offline"}
                      </p>
                    </div>
                  </div>
                );
