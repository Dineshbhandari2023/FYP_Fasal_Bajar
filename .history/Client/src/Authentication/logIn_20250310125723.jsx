  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        // Our backend returns an object in response.data.Result
        const { accessToken, refreshToken, user_data } = response.data.Result;

        // Dispatch the login action with the token and user details
        dispatch(login({ token: accessToken, user: user_data }));

        // Persist token and user data based on the rememberMe flag
        if (rememberMe) {
