...
export const fetchProducts = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const { url, method } = ApiLink.getAllProducts.url;
      const response = await axios({ url, method });
      // Extract product array from the nested response
...
export const createProduct = createAsyncThunk(
  ...
  async (productData, { rejectWithValue }) => {
    try {
      const { url, method } = ApiLink.createProduct.url;
      const response = await axios({
        url,
        method,
        data: productData,
      ...
      // Return the newly created product (assuming it's nested under Result.productData)
      return response.data.Result.productData;
    } catch (error) {
      return rejectWithValue(error.response?.data || error.message);
    }
...
export const updateProduct = createAsyncThunk(
  ...
  async ({ id, updatedData }, { rejectWithValue }) => {
    try {
      const { url, method } = ApiLink.updateProduct.url;
      const response = await axios({
        url: url(id), // pass the ID into the url function
        method,
        data: updatedData, // your updated product data
...
export const deleteProduct = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const { url, method } = ApiLink.deleteProduct.url;
      await axios({ url: url(id), method });
      // Return the deleted product's ID so the reducer can update the state
      return id;
    } catch (error) {
      return rejectWithValue(error.response?.data || error.message);
    }
