...
// Create a new order (returns full result including paymentUrl if applicable)
export const createOrder = createAsyncThunk(
  "orders/createOrder",
  async (orderData, thunkAPI) => {
    try {
      ...
      // Return the full result (order and paymentUrl if applicable)
      return response.data.Result;
    } catch (error) {
      return thunkAPI.rejectWithValue(
        error.response?.data?.ErrorMessage || error.message
...
export const fetchMyOrders = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      ...
      console.log("fetchMyOrders response:", response.data.Result.orders);
      return response.data.Result.orders;
    } catch (error) {
      return thunkAPI.rejectWithValue(
        error.response?.data?.ErrorMessage || error.message
...
export const fetchFarmerOrderItems = createAsyncThunk(
  "orders/fetchFarmerOrderItems",
  async (_, thunkAPI) => {
    try {
      const token = thunkAPI.getState().user.accessToken;
      const response = await axios.get("/orders/farmer-items", {
        headers: { Authorization: `Bearer ${token}` },
        withCredentials: true,
      });
      console.log("fetchFarmerOrderItems response:", response.data);
      return response.data.Result.orderItems;
    } catch (error) {
      console.error("fetchFarmerOrderItems error:", error);
      return thunkAPI.rejectWithValue(
        error.response?.data?.ErrorMessage || error.message
      );
    }
...
export const fetchFarmerOrders = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const token = thunkAPI.getState().user.accessToken;
      const response = await axios.get("/orders/farmer-orders", {
        headers: { Authorization: `Bearer ${token}` },
        withCredentials: true,
      });
      console.log("fetchFarmerOrders response:", response.data);
      return response.data.Result.orders || [];
    } catch (error) {
      console.error("fetchFarmerOrders error:", error.response?.data);
      return thunkAPI.rejectWithValue(
        error.response?.data?.errors?.body?.[0] || error.message
      );
    }
  }
);
...
const orderSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(createOrder.fulfilled, (state, action) => {
        state.loading = false;
        // If online payment, action.payload contains paymentUrl
      ...
      // Fetch farmer order items
      .addCase(fetchFarmerOrderItems.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchFarmerOrderItems.fulfilled, (state, action) => {
        state.loading = false;
        state.farmerItems = action.payload;
      })
      .addCase(fetchFarmerOrderItems.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })
      ...
      .addCase(fetchFarmerOrders.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
        state.farmerOrders = []; // Ensure farmerOrders is empty on error
      });
  },
});
