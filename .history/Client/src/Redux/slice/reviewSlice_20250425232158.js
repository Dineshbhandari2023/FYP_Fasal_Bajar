...
export const fetchFarmerReviews = createAsyncThunk(
  ...
  async (farmerId, { rejectWithValue }) => {
    try {
      ...
      console.log(`Fetching reviews for farmerId: ${farmerId}`);
      ...
      console.log("Fetched farmer reviews:", response.data);
      return response.data.Result.reviews; // Extract reviews array
    } catch (error) {
      console.error("Error fetching farmer reviews:", {
...
export const fetchFarmerRating = createAsyncThunk(
  ...
  async (farmerId, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(
        `${SERVER_URL}/reviews/farmer/${farmerId}/rating`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      console.log("Fetched farmer rating:", response.data);
      return response.data.Result; // Expect { averageRating, totalReviews }
    } catch (error) {
      console.error("Error fetching farmer rating:", {
        message: error.message,
        response: error.response,
        status: error.response?.status,
        data: error.response?.data,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchFarmerReviews.fulfilled, (state, action) => {
        state.loading = false;
        console.log("Updating reviews state with:", action.payload); // Debug log
        state.reviews = action.payload || []; // Ensure reviews is an array
      })
      ...
      .addCase(fetchFarmerRating.fulfilled, (state, action) => {
        state.loading = false;
        console.log("Updating rating state with:", action.payload); // Debug log
        state.rating = action.payload || { averageRating: 0, totalReviews: 0 };
      })
