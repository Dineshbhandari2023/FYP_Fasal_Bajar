...
const Settings = () => {
  ...
  // Initial form data; will be updated once user info is fetched
  const [formData, setFormData] = useState({
    fullName: "",
    email: "",
    role: "",
      ...
      axios
        ...
        .then((response) => {
          if (response.data && response.data.user) {
            const { username, email, role, location } = response.data.user;
            ...
            // Also update original data so that cancelEdit works properly
            setOriginalData({
              fullName: username,
              email: email,
              role: role,
  ...
  const handleSubmit = async (e) => {
    ...
    // Note: Your update route expects a "user" object in the body.
    const updatePayload = {
      user: {
    ...
    try {
      ...
      if (response.data && response.data.user) {
        // Update state with the returned updated user details.
        setUser(response.data.user);
        setFormData((prevData) => ({
          ...prevData,
