...
const Settings = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    // Create FormData so we can send the file (if any) along with text fields.
    const data = new FormData();
    data.append("username", formData.username);
    data.append("email", formData.email);
    data.append("contact_number", formData.contact_number);
    data.append("location", formData.location);
    // Append new profile image if available. Backend expects field "image"
    if (newProfileImage) {
  ...
  // Helper: construct image URL if profileImage exists.
  // Assumes that userInfo.profileImage is a base64 encoded string.
