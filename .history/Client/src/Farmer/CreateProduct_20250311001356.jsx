...
const CreateProduct = () => {
  ...
  const { token } = useSelector((state) => state.auth); // Grab token if needed for API calls
  ...
  const handleSubmit = (e) => {
    ...
    // Backend expects "images" (plural) field for files:
    if (formData.imageFile) {
      data.append("images", formData.imageFile);
    }

    dispatch(createProduct({ data, token })) // token included if you need to pass it for auth headers
      .unwrap()
      .then((createdProduct) => {
        ...
        setErrors({});
        dispatch(fetchProducts()); // Refresh product list after successful creation
      })
      .catch((err) => {
