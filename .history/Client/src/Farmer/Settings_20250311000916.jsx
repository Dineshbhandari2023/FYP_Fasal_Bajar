...
const Settings = () => {
  ...
  // Grab the logged-in user and token from Redux
    ...
    const fetchUserById = async () => {
      ...
      try {
        const response = await axios.get(
          `http://localhost:8000/users/${loggedInUser.id}`,
          {
            headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!token) {
      console.error("No token found for updating user");
      return;
    }
    ...
    try {
      ...
      if (response.data && response.data.Result.user_data) {
