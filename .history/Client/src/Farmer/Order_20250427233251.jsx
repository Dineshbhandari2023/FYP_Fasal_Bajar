...
const Order = () => {
  ...
  useEffect(() => {
    console.log("Order.jsx: userLoading:", userLoading);
    console.log("Order.jsx: accessToken:", accessToken);
    console.log("Order.jsx: user:", user);
    console.log("Order.jsx: user.role:", user.role);

    if (userLoading) {
      console.log("Order.jsx: Waiting for user data to load");
      return;
    }

    if (!accessToken) {
      console.log("Order.jsx: Redirecting to /login - No accessToken");
      navigate("/login");
    } else if (!user || user.role !== "Farmer") {
      console.log("Order.jsx: Redirecting to / - Not a farmer, user:", user);
      navigate("/");
    } else {
      console.log("Order.jsx: Fetching farmer orders");
      dispatch(fetchFarmerOrders());
    }
  }, [dispatch, accessToken, user, userLoading, navigate]);

  // Flatten OrderItems for display
  const flattenedItems = farmerOrders.flatMap((order) =>
  ...
  // Filter items based on status, date, product, and price
