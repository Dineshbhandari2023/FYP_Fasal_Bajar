...
class WebSocketLocationService {
  ...
  .onSupplierLocationUpdate((data) => {
    const id = Number(data.supplierId);        // ← cast to number
    this.callbacks.onSupplierLocationUpdate({ id, ...data });
  })
  ...
  onActiveSuppliersList((suppliersArray) => {
    ...
    suppliersArray.forEach((sup) => {
      const id = Number(sup.supplierId);       // ← cast to number
      mapObj[id] = {
    ...
    this.callbacks.onActiveSuppliersList(mapObj);
  })
