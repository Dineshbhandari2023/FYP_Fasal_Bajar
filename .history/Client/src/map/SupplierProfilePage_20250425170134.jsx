...
const SupplierProfile = () => {
  ...
  // Get userInfo and accessToken from Redux state
  const { userInfo, accessToken } = useSelector((state) => state.user || {});
  console.log("User info:", userInfo);
  console.log("Access token:", accessToken ? "Present" : "Missing");

  // Use a safe selector for reviews
  const {
    reviews = [],
  ...
  const { suppliers = [], loading: suppliersLoading = false } = useSelector(
  ...
  // Find supplier from Redux state or fetch individually
  useEffect(() => {
    if (suppliers?.length > 0) {
      const foundSupplier = suppliers.find(
        (s) => s.id === Number.parseInt(supplierId)
      );
      if (foundSupplier) {
    ...
    } else if (!suppliersLoading) {
      fetchSupplierDetails();
    }
  }, [supplierId, suppliers, suppliersLoading]);

  // Fetch supplier details if not in Redux state
  ...
  useEffect(() => {
    if (supplierId) {
      dispatch(fetchFarmerReviews(supplierId)); // Assuming reviews are for both farmers and suppliers
      dispatch(fetchFarmerRating(supplierId));
    }
  }, [dispatch, supplierId]);
  ...
  useEffect(() => {
    if (supplierId) {
      socketService.joinFarmerReviewRoom(supplierId); // Reusing for suppliers
    }

    return () => {
      if (supplierId) {
        socketService.leaveFarmerReviewRoom(supplierId);
      }
    };
  }, [supplierId]);
  ...
  const submitReviewDirectly = async (reviewData) => {
    try {
      ...
      dispatch(fetchFarmerReviews(supplierId));
      dispatch(fetchFarmerRating(supplierId));
  ...
  const handleSubmitReview = async (e) => {
    ...
    if (editingReview) {
      ...
      const reviewData = {
        supplierId, // Changed from farmerId to supplierId
        userId: userInfo.id,
      ...
      dispatch(submitReview(reviewData)).then((action) => {
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-3">
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="text-gray-700">
                      Vehicle: {supplier.SupplierDetail?.vehicleType || "N/A"}
                    </span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="text-gray-700">
                    {supplier.SupplierDetail?.serviceArea || "Not specified"}
                  </p>
                </div>
              </div>
