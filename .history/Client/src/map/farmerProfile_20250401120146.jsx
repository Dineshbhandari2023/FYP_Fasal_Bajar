...
const FarmerProfile = () => {
  ...
  // Redux state - with fallback values to prevent errors
  ...
  const [forceShowReviewButton, setForceShowReviewButton] = useState(false); // New state to force show the review button
  ...
  // Check if reviews are loaded and if user has already reviewed
  useEffect(() => {
    if (reviews.length > 0 && currentUser) {
      // Force update the review button visibility after reviews are loaded
      setForceShowReviewButton(true);
    }
  }, [reviews, currentUser]);
  ...
  const isBuyer =
    currentUser &&
    (currentUser.role === "Buyer" || currentUser.role === "buyer");
      ...
      <div className="container mx-auto px-4 py-6 md:py-8">
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden mb-6">
          <div className="p-6">
            ...
            {currentUser &&
              isBuyer &&
              (showReviewForm || forceShowReviewButton) &&
              !editingReview && (
