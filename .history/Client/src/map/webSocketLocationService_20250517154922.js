...
const SERVER_URL = process.env.REACT_APP_WS_URL || "http://localhost:8000";
...
class WebSocketLocationService {
  ...
  registerAsSupplier(supplierId, username, serviceArea) {
    if (!this.connected || !this.authenticated) {
      console.warn(
        "Cannot register: WebSocket not connected or not authenticated"
      );
      this.callbacks.onError({
        type: "auth",
  ...
  updateLocation(
    ...
    if (!this.connected || !this.authenticated) {
      console.warn(
        "Cannot update location: WebSocket not connected or not authenticated"
      );
      this.callbacks.onError({
        type: "auth",
        message: "Not authenticated or socket not connected",
  ...
  setSupplierOffline(supplierId) {
    if (!this.connected || !this.authenticated) {
      console.warn(
        "Cannot set offline: WebSocket not connected or not authenticated"
      );
      return false;
    }
  ...
  getActiveSuppliers() {
    if (!this.connected || !this.authenticated) {
      console.warn(
        "Cannot get active suppliers: WebSocket not connected or not authenticated"
      );
      return false;
    }
