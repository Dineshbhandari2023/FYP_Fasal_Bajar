...
const SupplierProfilePage = () => {
  ...
  // Safely access the reviews state with fallbacks for undefined values
  ...
  const fetchSupplierDetails = async () => {
    try {
      ...
      // If it's an auth error, we'll handle it differently
      if (
  ...
  useEffect(() => {
    ...
    if (supplierId) {
      ...
      // Only dispatch these actions if we have a valid token
      if (!authError) {
  ...
  }, [dispatch, supplierId, userInfo, navigate, authError]);

  useEffect(() => {
    if (supplierId && !authError) {
      socketService.joinSupplierReviewRoom(supplierId);
      return () => {
        socketService.leaveSupplierReviewRoom(supplierId);
      };
    }
  }, [supplierId, authError]);
