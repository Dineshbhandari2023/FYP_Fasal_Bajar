...
const Map = () => {
  ...
  const { farmersList, loading: farmersLoading, error: farmersError } =
    useSelector((state) => state.user);
  const { suppliers = [], loading: suppliersLoading, error: suppliersError } =
  ...
  useEffect(() => {
    const totalResults = (filteredFarmers?.length || 0) + (filteredSuppliers?.length || 0);
      ...
      <div className="container mx-auto px-4 py-4 md:py-8">
        <div className="mb-4 md:mb-8">
          ...
          <p className="text-sm md:text-base text-gray-600 max-w-2xl">
            Connect directly with farmers and suppliers in your area to get fresh,
            locally grown produce or reliable delivery services. Browse the map or
            list below to find them near you.
          </p>
        </div>
        ...
        >
          {!farmersLoading && !suppliersLoading && !farmersError && !suppliersError && (
            <MapContainer
              center={mapCenter}
              zoom={zoom}
                  ...
                  >
                    <Popup>
                      <div className="text-center p-1">
                        ...
                        <button
                          className="mt-1 text-xs text-green-600 hover:text-green-800"
                          onClick={() => handleMarkerClick(supplier, "supplier")}
                        >
                          View Details
                        </button>
                      </div>
            ...
            <div className="text-center mb-4 sm:mb-6">
              <div
                className={`w-24 h-24 sm:w-32 sm:h-32 rounded-full ${
                  entityType === "farmer" ? "bg-green-100 border-green-500" : "bg-blue-100 border-blue-500"
                ...
                {selectedEntity.profileImage ? (
                  ...
                  <User
                    className={`h-12 w-12 sm:h-16 sm:w-16 ${
                      entityType === "farmer" ? "text-green-600" : "text-blue-600"
                    }`}
                  />
                )}
              </div>
