...
const Map = () => {
  ...
  useEffect(() => {
    locationService
      ...
      .onSupplierLocationUpdate((data) => {
        ...
        // Update the supplier's location in our local state
  ...
  const hasRealtimeLocation = (supplierId) => {
    return realtimeSuppliers[supplierId] !== undefined;
  };
  ...
  const isRealtimeRecent = (supplierId) => {
    if (!realtimeSuppliers[supplierId]) return false;

    const lastUpdate = new Date(realtimeSuppliers[supplierId].lastUpdated);
  ...
  const filteredSuppliers = suppliers?.filter((supplier) => {
    ...
    if (showRealtimeOnly) {
      return (
        matchesSearch &&
        hasRealtimeLocation(supplier.id) &&
        isRealtimeRecent(supplier.id)
      );
    }
                ...
                {entityType === "supplier" && (
                  <div className="mt-1">
                    ...
                    isRealtimeRecent(selectedEntity.id) ? (
                      <span className="inline-flex items-center text-green-600 bg-green-50 px-2 py-1 rounded-full text-sm">
                        <Wifi className="h-4 w-4 mr-1" />
                        Live Tracking Active
                      </span>
                    ) : (
                      <span className="inline-flex items-center text-gray-600 bg-gray-100 px-2 py-1 rounded-full text-sm">
